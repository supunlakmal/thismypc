...
var pcSchema = mongoose.Schema({
    pcKey: {
        ...
        type: String, default: Date.now, unique: true
    }, publicAccessStatus: {
...
module.exports.getPCByUserIDAndPCKey = function (key, userID, callback) {
    PC.findOne().where({"pcKey": key, 'userID': userID}).exec(callback);
}
//  get all  pc using  user id
module.exports.getPCByUserID = function (userID, callback) {
    PC.find().where({"userID": userID, 'status': 1}).exec(callback);
}
//  get all online  pc using  user id
module.exports.getPCByUserIDOnline = function (userID, callback) {
    PC.find().select('_id pcName platform email').where({"userID": userID, 'status': 1, 'pcOnline': 1}).exec(callback);
}//  get   pc using  pc id
module.exports.getPCByPCID = function (pcID, callback) {
    PC.findOne().where({"pcID": pcID, 'status': 1}).exec(callback);
}
...
module.exports.updatePublicAccessStatus = function (pcID, pc, option, callback) {
    var query = {_id: pcID};
    var update = {
        publicAccessStatus: pc.publicAccessStatus, publicAccessKey: pc.publicAccessKey
    }
    PC.findOneAndUpdate(query, update, option, callback);
}
...
module.exports.updatePcOnlineStatus = function (pcID, pc, option, callback) {
    var query = {_id: pcID};
    var update = {
        pcOnline: pc.pcOnline, pcSocketID: pc.pcSocketID
    }
    PC.findOneAndUpdate(query, update, option, callback);
}
...
module.exports.updateUserAuthApp = function (pcKey, pc, option, callback) {
    var query = {pcKey: pcKey, userID: pc.id};
    var update = {
        authApp: pc.auth
    }
    /*    var update = {
...
module.exports.authApp = function (id, auth,pcKey, callback) {
    ...
    PC.findOne().where({"userID": id, "authApp": auth , 'pcKey':pcKey}).exec(callback);
}
...
module.exports.countPC = function (callback) {
    PC.findOne().where({ 'status': 1}).count(callback);
}
