...
module.exports.createNewUserAndApp = function (userApp, callback) {
    UserAndApp.create({'userID': userApp.id, 'appID': userApp.appID}, callback);
}// delete   User and app
module.exports.deleteUserAndApp = function (userApp, callback) {
    UserAndApp.remove({'userID': userApp.id, 'appID': userApp.appID}, callback);
}
...
module.exports.userApps = function (userApp, callback) {
    UserAndApp.aggregate([{
        ...
        '$lookup': {
            'from': 'apps', 'localField': 'appID', 'foreignField': '_id', 'as': 'apps'
        }
    }], callback);
}
//  get  app using userID and  app  ID
module.exports.getUserAndApp = function (data, callback) {
    UserAndApp.findOne().where({"userID": data.id, "appID": data.appID}).exec(callback);
}
