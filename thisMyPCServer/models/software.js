...
module.exports.createSoftwareVersion = function (software, callback) {
    Software.create({'version': software.version, 'versionKey': software.versionKey, 'status': software.status}, callback);
}
...
module.exports.getActiveSoftware = function (key, callback) {
    Software.findOne().where({"versionKey":key , 'status':1} ).exec(callback);
}
